...
public class MediaStringUtils {
    ...
    private static final String TAG = MediaStringUtils.class.getSimpleName();
     ...
     * 根据秒速获取时间格式
     */
    public static String gennerMinSec(int totalSeconds) {
        int seconds = totalSeconds % 60;
        int minutes = (totalSeconds / 60) % 60;
        return minutes > 0 ? seconds > 0 ? String.format(Locale.getDefault(), "%2d分钟%2d秒", minutes, seconds) : String.format(Locale.getDefault(), "%2d分钟", minutes)
                : String.format(Locale.getDefault(), "%2d秒", seconds);
    }

    public static String gennerTimeSecond(int totalSeconds) {
        int seconds = totalSeconds % 60;
    ...
    public static String substring(String search, String start, String end,
        ...
        int start_pos = MediaStringUtils.isEmpty(start) ? 0 : search.indexOf(start);
        if (start_pos > -1) {
            int end_pos = MediaStringUtils.isEmpty(end) ? -1 : search.indexOf(end,
                    start_pos + start_len);
            if (end_pos > -1)
