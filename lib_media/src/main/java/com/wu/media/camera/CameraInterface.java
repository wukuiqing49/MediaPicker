...
import com.wu.media.camera.util.CheckPermission;
...
//            ObjectAnimator animF = ObjectAnimator.ofFloat(mFlashLamp, "rotation", start_rotaion, end_rotation);
    ...
    public void setFlashMode(String flashMode) {
        ...
        try {
            ...
            } else if (flashMode.contains(Camera.Parameters.FLASH_MODE_OFF)) {
    ...
    private synchronized boolean openCamera(int id) {
        try {
            ...
            if (this.errorLisenter != null) this.errorLisenter.onError();
            Log.e("CJT", "enable shutter sound faild", e);
        }
    ...
    public synchronized boolean switchCamera(SurfaceHolder holder, float screenProp) {
     ...
     * doStartPreview
     */
    public boolean doStartPreview(SurfaceHolder holder, float screenProp) {
        if (isPreviewing) {
        }
...
//        }
            ...
            Log.i("CJT", "startRecord Exception");
        }
        return false;
    }
    ...
    private void findAvailableCameras() {
        ...
        for (int i = 0; i < cameraNum; i++) {
            ...
            try {
                ...
                Log.e("CameraInterface", "相机信息获取异常", e);
                continue;
            }
            switch (info.facing) {
