...
import static com.wu.media.camera.JCameraView.BUTTON_STATE_BOTH;
import static com.wu.media.camera.JCameraView.BUTTON_STATE_ONLY_CAPTURE;
import static com.wu.media.camera.JCameraView.BUTTON_STATE_ONLY_RECORDER;
...
public class CaptureButton extends View {
    ...
    private int progress_color = Color.GREEN;            //进度条颜色
    private int outside_color = 0xEEf2f2f2;             //外圆背景色
    private int inside_color = Color.WHITE;              //内圆背景色
    ...
    public CaptureButton(Context context, int size) {
        ...
        min_duration = 2000;              //默认最短录制时间为1.5s
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if ((button_state == BUTTON_STATE_ONLY_RECORDER || button_state == BUTTON_STATE_BOTH))
                    postDelayed(longPressRunnable, 50);    //同时延长500启动长按后处理的逻辑Runnable
                break;
