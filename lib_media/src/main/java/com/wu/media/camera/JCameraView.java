...
//    private static final String TAG = "JCameraView";
    ...
    public JCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        // iconSrc = a.getResourceId(R.styleable.JCameraView_iconSrc, R.drawable.fb_xiangji);
    ...
    private void initView() {
        ...
        mVideoView.setLayoutParams(new LayoutParams(getScreenWidth(), getScreenWidth() * 1920 / 1080));
        mPhoto = (ImageView) view.findViewById(R.id.image_photo);
        mSwitchCamera = (ImageView) view.findViewById(R.id.image_switch);
        mSwitchCamera.setImageResource(R.drawable.fb_xiangji);
        ...
        if (System.currentTimeMillis() - lastClick <= 4000) {
            ...
            mFlashLamp.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (CameraInterface.getInstance().getSelectedCamera() == Camera.CameraInfo.CAMERA_FACING_FRONT)
                        return;
                    type_flash++;
                    if (type_flash > 0x023) {
                        type_flash = 0x022;
                    }
                    if (type_flash == TYPE_FLASH_ON) {
                        mFlashLamp.setImageResource(R.drawable.ic_flash_on);
                        machine.flash(Camera.Parameters.FLASH_MODE_ON);
                    } else if (type_flash == TYPE_FLASH_OFF) {
                        mFlashLamp.setImageResource(R.drawable.ic_flash_off);
                        machine.flash(Camera.Parameters.FLASH_MODE_OFF);
                    } else {
                        Toast.makeText(getContext(),"闪光灯打开失败", Toast.LENGTH_SHORT).show();
                        mFlashLamp.setImageResource(R.drawable.ic_flash_off);
                        machine.flash(Camera.Parameters.FLASH_MODE_OFF);
                    }
                }
            ...
            mSwitchCamera.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (System.currentTimeMillis() - lastClickTime < FAST_CLICK_DELAY_TIME) {
                        ...
                        if (machine.swtich(mVideoView.getHolder(), screenProp)) {
                            if (cid[0] == 1) {
        ...
        mCaptureLayout.setCaptureLisenter(new CaptureListener() {
            ...
            public void takePictures() {
                if (machine.capture()) {
                    ...
                    Toast.makeText(getContext(),"相机拍摄失败", Toast.LENGTH_SHORT).show();
                    Log.e("Camera", "相机拍摄失败.");
                    mCaptureLayout.onError();
                    recordEnd = true;
                }
            ...
            public void recordStart() {
                recordEnd = false;
                if (machine.record(mVideoView.getHolder().getSurface(), screenProp,getContext())) {
                    mSwitchCamera.setVisibility(INVISIBLE);
                    mFlashLamp.setVisibility(INVISIBLE);
                } else {
                    Toast.makeText(getContext(),"相机开启失败", Toast.LENGTH_SHORT).show();
                    Log.e("Camera", "相机开启失败.");
                    mCaptureLayout.onError(true);
                }
            }
            ...
            public void recordEnd(long time) {
                ...
                if (jCameraLisenter != null) {
                    jCameraLisenter.recordEnd(videoUrl, duration);
                }
            }
...
//        });
    ...
    public void setDuration(int duration_ss) {
        if (mCaptureLayout != null) mCaptureLayout.setDuration(duration = duration_ss);
    }
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public void confirmState(int type) {
        switch (type) {
            case TYPE_VIDEO:
                ...
                machine.start(mVideoView.getHolder(), screenProp);
                if (jCameraLisenter != null) {
                    jCameraLisenter.recordSuccess(videoUrl, duration);
                }
                break;
            case TYPE_PICTURE:
                mPhoto.setVisibility(INVISIBLE);
    ...
    public void showPicture(Bitmap bitmap, boolean isVertical) {
        ...
        mPhoto.setImageBitmap(captureBitmap);
    ...
    public void onDestroy() {
        ...
        if (machine != null) machine.onDestroy();
    }
}
